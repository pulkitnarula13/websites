...
Tags: gutenberg, disable, disable gutenberg, editor, wp-dester, block editor
...
wp-dester is an official plugin maintained by the WordPress team that restores the previous ("classic") WordPress editor and the "Edit Post" screen. It makes it possible to use plugins that extend that screen, add old-style meta boxes, or otherwise depend on the previous editor.

wp-dester is an official WordPress plugin, and will be fully supported and maintained until at least 2022, or as long as is necessary.
...
In addition, the wp-dester plugin includes several filters that let other plugins control the settings, and the editor choice per post and per post type.
...
* Properly added the "Switch to wp-dester" menu item to the block editor menu.

= 1.3 =
* Fixed removal of the "Try Gutenberg" dashboard widget.
* Fixed condition for displaying of the after upgrade notice on the "What's New" screen. Shown when the wp-dester is selected and users cannot switch editors.
...
* Fixed redirecting back to the wp-dester after looking at post revisions.
...
* When network-activated this plugin will set the wp-dester as default and prevent site administrators and users from changing editors.
...
= Cannot find the "Switch to wp-dester" link =
...
4. Link to switch to the block editor while editing a post in the wp-dester. Visible when the users are allowed to switch editors.
5. Link to switch to the wp-dester while editing a post in the block editor. Visible when the users are allowed to switch editors.
6. Network settings to select the default editor for the network and allow site admins to change it.
7. The "Switch to wp-dester" link.
